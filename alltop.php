						...
						case '起床':
							...
							if(!empty($rs)&&$rs==1){
							 ...
							 if ($time>=6&&$time<8)//设置起床报道时间，
								...
								{
								  $str=$name."，现在离起床还早呢，再睡一会吧！"."六点到八点来哦";//这里是如果起床时间早于4点，进行的提示
								  } 
								  else if($time>=8&&$time<=19)
								  {
								  $str=$name."，亲，小伙伴们已经起床了呢，你迟到了哦！"."六点到八点再来哦~";//这里是如果起床时间是中午12点过后，进行的提示
								  } 
									  else {
										$str=$name."，太阳都下山啦！你还是明天再来吧！"."六点到八点再来哦~";//这里是如果起床时间19点后，进行的提示
									 ...
									 $str="请输入\"叫我 姓名\"进行绑定，如：叫我 MT。一定要在中间加空格哦！绑定能参与早起大作战哦！";//这里是如果没有进行绑定，进行的提示
          ...
          $rs_name= mysql_fetch_array($query_name);
           $str="绑定成功，您的昵称是：$rs_name[name]；请输入起床，参与四川大学排名争霸！";//这里是绑定成功，进行的提示
					  ...
					  $str="你的积分是：".$rs[integral];
						...
						break;
						case '帮我起床':
						$str="亲爱的小伙伴们，假期一定玩的很愉快吧~不过还是保持一定的早起时间比较好，早睡早起身体好，头不晕，眼不胀，干起家务活来麻溜利索的，麻麻再也不用担心自家的宝贝是懒猫了~\n";
						break;
                      case '修改昵称':
